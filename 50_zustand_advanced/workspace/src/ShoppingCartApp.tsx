...
function useRemoveProduct() {
  ...
  const removeCartItem = useAppStore((s) => s.updateItemQuantity);

  const handleRemoveProduct = (productId: string) => {
    removeProduct(productId);
    // ....
    removeCartItem(productId, -100000);
  };
...
function Product({ product }: ProductProps) {
  ...
  //   removeCartItem(product.id, -100000);
  // };
  ...
  // ANSATZ-3
  const removeProduct = useAppStore((s) => s.removeProductFromStore);
  const handleRemoveProduct = () => {
    removeProduct(product.id);
  };

  const addToCart = useAppStore((s) => s.shoppingCart.addItem);
