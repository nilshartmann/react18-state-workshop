...
function ShoppingCart() {
  ...
  const productIds = useAppStore(
    useShallow((s) => s.cart.items.map((i) => i.productId)),
  );
...
function ShoppingCartItem({ productId }: ShoppingCartItemProps) {
  ...
  const productName = useAppStore(
    (s) =>
      s.shop.products.find((p) => p.id === productId)?.label ||
      "unknown product",
  );
  const updateItemQuantity = useAppStore(
    (state) => state.cart.actions.updateItemQuantity,
  );
...
function ProductList() {
  const products = useAppStore((s) => s.shop.products);
...
function Product({ product }: ProductProps) {
  const { increasePrice, removeProduct } = useAppStore((s) => s.shop.actions);
  const { addItem } = useAppStore((s) => s.cart.actions);

  const handleIncreasePrice = () => {
    increasePrice(product.id, 0.5);
  };

  const handleAddToCart = () => addItem(product.id);
